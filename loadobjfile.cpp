...
int
LoadObjFile( char *name , std::vector<Vertex*> *polyList)
{
	...
	for( ; ; )
	{
		...
		if( strcmp( cmd, "f" )  ==  0 )
		{
			...
			for( int it = 0; it < numTriangles; it++ )
			{
				...
				//the vertices for this triangle of the polygon
				struct Vertex* thisTri = new Vertex[3];

				for( int vtx = 0; vtx < 3 ; vtx++ )
				{

					//copy the vertices to the return polygon
					thisTri[vtx].x = Vertices[vertices[vv[vtx]].v - 1].x;
					thisTri[vtx].y = Vertices[vertices[vv[vtx]].v - 1].y;
					thisTri[vtx].z = Vertices[vertices[vv[vtx]].v - 1].z;
